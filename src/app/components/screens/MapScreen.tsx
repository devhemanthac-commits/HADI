...
const ALL_CATEGORIES: PlaceCategory[] = ["Heritage", "Temple", "Church", "Food", "Stay", "Nature", "Hidden Gem"];
...
function makeIcon(emoji: string, color: string, selected: boolean): L.DivIcon {
  ...
  const pulse = selected
    ...
    : "";
  return L.divIcon({
    html: `<div style="position:relative;display:flex;align-items:center;justify-content:center;width:${size}px;height:${size}px;border-radius:50%;background:${color};box-shadow:${shadow};border:${selected ? "3px" : "2.5px"} solid #fff;font-size:${selected ? 22 : 18}px;cursor:pointer;transition:all 0.2s;">${pulse}${emoji}</div>`,
    className: "",
    iconSize: [size, size],
...
export function MapScreen() {
  ...
  useEffect(() => {
    ...
    if (mapTab === "Explore") {
      filtered.forEach((place) => {
        ...
        const icon = makeIcon(place.emoji, catConfig[place.category].color, isSelected);
  ...
  }, [mapTab, activeCategory, filtered.length]);
  ...
  useEffect(() => {
    ...
    markersRef.current.forEach(({ marker, id }) => {
      ...
      if (!place) return;
      marker.setIcon(makeIcon(place.emoji, catConfig[place.category].color, selected?.id === id));
    });
  }, [selected, mapTab]);

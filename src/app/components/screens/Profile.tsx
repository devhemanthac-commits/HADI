...
export function Profile() {
  ...
  const C = useColors();
  const { darkMode, toggleDarkMode, localMode, toggleLocalMode, savedGems, toggleSaved, userName, setUserName } = useApp();
  ...
  return (
    <div className="animate-fade-up flex flex-col gap-6">
      ...
      {/* Profile Header */}
      <div className="rounded-[28px] overflow-hidden relative" style={{ background: "linear-gradient(135deg, #0F3D3D 0%, #0A2A2A 100%)" }}>
      ...
      {activeTab === "Collections" && (
        <section>
          {collectionGems.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 gap-4 text-center">
              <span style={{ fontSize: 48 }}>🔖</span>
              <p className="font-playfair" style={{ color: C.text, fontSize: 18, fontWeight: 700 }}>No saved gems yet</p>
              <p className="font-dm" style={{ color: C.muted, fontSize: 13 }}>Tap the bookmark icon on any gem to save it here</p>
            ...
            <>
              <p className="font-dm mb-4" style={{ color: C.muted, fontSize: 13 }}>{collectionGems.length} saved gem{collectionGems.length !== 1 ? "s" : ""}</p>
              <div className="grid grid-cols-3 gap-3">
        ...
        <div className="rounded-[20px] overflow-hidden" style={{ background: C.card, border: `1px solid ${C.border}` }}>
          ...
          <div className="flex items-center justify-between px-5 py-4">
            <div className="flex items-center gap-3">
              <span style={{ fontSize: 20 }}>👤</span>
              <div>
                <p className="font-dm" style={{ color: C.text, fontWeight: 600, fontSize: 14 }}>Account</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)" }}>
          <div className="rounded-[28px] w-full max-w-[400px] flex flex-col sheet-enter overflow-hidden" style={{ background: C.bg, maxHeight: "90vh", boxShadow: "0 12px 48px rgba(0,0,0,0.3)" }}>
            ...
            <div className="p-5" style={{ borderTop: `1px solid ${C.border}` }}>
              <button onClick={() => { if (displayName.trim()) setUserName(displayName.trim()); setShowEditModal(false); }} className="font-dm w-full pressable"
                style={{ height: 48, borderRadius: 99, background: "#0F3D3D", color: "#E07B2A", fontSize: 15, fontWeight: 700, border: "none", cursor: "pointer", boxShadow: "0 4px 14px rgba(15,61,61,0.2)" }}>
                Save Profile
              </button>
            </div>
          </div>
